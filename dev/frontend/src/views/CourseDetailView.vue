        ...
        <div v-for="(slots, day) in slotsByDay" :key="day" style="margin-bottom: 2.5rem;">
          ...
          <div class="slot-board">
            <div v-for="slot in slots" :key="slot.sequence" class="slot-card" :class="{ locked: slot.is_locked, 'spinning-anim': courseStore.spinningSlots[`${slot.day_number}_${slot.sequence}`] }" @click="openPlaceModal(slot.place)">
              ...
              <p style="font-size: 0.85rem; color: var(--text-muted); margin-bottom: 0.4rem;">{{ slot.place.description }}</p>
              <a v-if="slot.place.place_url" :href="slot.place.place_url" target="_blank" @click.stop style="font-size: 0.8rem; color: var(--color-accent); font-weight: bold; text-decoration: none; display: inline-block;">카카오맵에서 보기 ↗</a>
            </div>
          </div>
        </div>
      </div>
