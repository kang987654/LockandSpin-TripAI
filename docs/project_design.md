...
### 3.1. Re-spin 및 동적 라우팅 동작 원리 (Backend Flow)
1. **요청 수신**: 프론트엔드에서 현재 편집 중인 `course_id`와 함께 현재 메모리에 렌더링된 슬롯 정보 리스트(각 슬롯의 `place_id`, `day_number`, `sequence`, `is_locked` 상태)를 JSON으로 전송합니다.
2. **고정 슬롯 식별**: 백엔드는 전송된 리스트 중 `is_locked=True`인 슬롯을 고정 대상으로 보존합니다.
3. **위치 기반 동적 라우팅 가중치 계산 (Dynamic Routing with Map Data)**:
   * 잠금 처리된 장소들의 위도(`latitude`)와 경도(`longitude`) 값을 순서대로 추출합니다.
   * 비어있는(미고정) 슬롯들의 앞뒤에 인접한 잠금 장소가 있다면, 해당 좌표 간 **하버사인(Haversine) 공식** 또는 **실제 경로 API**를 모방하여 이동 예상 거리가 짧은 최적의 `Place` 후보들을 상위에 필터링합니다.
4. **필터링 & 재추천**:
   * 사용자 비토 카테고리(`UserVetoCategory`)에 속한 장소는 후보군에서 배제합니다.
   * `UserPreference` 태그와의 매칭율을 고려해 대체 후보지를 무작위/가중치 방식으로 선정하여 미고정 슬롯을 채웁니다.
5. **실시간 동기화 전송 (WebSocket Sync)**:
   * 백엔드는 업데이트된 코스를 DB에 즉시 저장하고, 해당 코스에 참여 중인 모든 공동 멤버(`CourseMember`)의 WebSocket 채널을 통해 변경된 슬롯 상태를 브로드캐스트하여 화면을 실시간 업데이트합니다.
